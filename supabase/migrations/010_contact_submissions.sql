-- Contacto público: mensajes + adjuntos cifrados en cliente (AES-GCM).
-- Solo el fundador (service role / políticas admin) puede leer.
...
-- Nadie autenticado como usuario normal puede leer/escribir vía anon key.
-- Las inserciones se hacen con service role desde el server action.
...
-- Storage bucket privado para ciphertext (solo service role).
...
-- Sin políticas de storage para authenticated/anon → solo service role.
