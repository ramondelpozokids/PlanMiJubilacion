...
export function ConsultationManager({
  ...
  cases: { id: string; clientName: string }[];
  uploadOnly?: boolean;
}) {
  ...
  return (
    <div className={uploadOnly ? 'space-y-4' : 'grid gap-6 lg:grid-cols-2'}>
      {!uploadOnly && (
      <form onSubmit={onCreate} className="rounded-xl border p-5 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Para amigos o familiares: sube su vida laboral, bases, simulación y nómina. No se cobra.
        </p>
      ...
      <form onSubmit={onUpload} className="rounded-xl border p-5 space-y-4">
        <h2 className="font-semibold">
          {uploadOnly ? 'Subir más documentos' : 'Subir documentos del tercero'}
        </h2>
        <label className="block text-sm">
